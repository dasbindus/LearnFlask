...
def followers(username):
    ...
    if user is None:
        ...
        return redirect(url_for('.index'))
    page = request.args.get('page', 1, type=int)
    pagination = user.followers.paginate(page, per_page=current_app.config['MYBLOG_FOLLOWERS_PER_PAGE'],
        error_out=False)
...
def followed_by(username):
    ...
    if user is None:
        ...
        return redirect(url_for('.index'))
    page = request.args.get('page', 1, type=int)
    pagination = user.followed.paginate(page, per_page=current_app.config['MYBLOG_FOLLOWERS_PER_PAGE'],
        error_out=False)
